defmodule OnboardingCm.Integration.CollectionManagementTest do
  ...
  describe "fluxo completo de gestão de coleções" do
    test "criar uma coleção completa com produtos e tags" do
      IO.puts("=========== Iniciando teste de CollectionManagement ===========")
      ...
      price_tag = TagContext.create!(%{name: "Faixa", value: "199,00", description: "Preço"})

      public_tag =
        TagContext.create!(%{name: "Público", value: "Adulto", description: "Segmentação"})

      material_tag =
        TagContext.create!(%{name: "Material", value: "Algodão", description: "Composição"})
